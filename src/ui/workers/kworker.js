...
function disassemble(inputFile, outputDir, roundtrip, overwrite) {
    ...
    if (overwrite) fs.writeFileSync(outPath, result.data);
    else {
        if (fs.existsSync(outPath))
            outPath = outPath.replace(/\.j$/, '_' + (new Date().getTime()) + '.j');
...
onmessage = (event) => {
    try {
        switch (event.data.op){
            case 'disassemble':
                postMessage({ success:true, result:disassemble(event.data.inputFile, event.data.outputDir, event.data.roundtrip, event.data.overwrite) });
                break;
            case 'assemble':
